    ...
    <div className="space-y-5 pb-10">
      ...
      <div className="grid gap-5 lg:grid-cols-[1.15fr_.85fr]">
        <section className="rounded-2xl border border-black/8 bg-white/75 p-5 shadow-sm backdrop-blur-xl dark:border-white/10 dark:bg-white/5" aria-labelledby="quote-title">
          <div className="flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.16em] text-stone-500"><Quote className="size-4" /> The principle</div>
          ...
          <p className="mt-4 text-xs text-stone-500">{state.heroQuote.author}</p>
        </section>
        <section className="rounded-2xl border border-black/8 bg-white/75 p-5 shadow-sm backdrop-blur-xl dark:border-white/10 dark:bg-white/5" aria-labelledby="month-title">
          <div className="flex items-center justify-between"><div className="flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.16em] text-stone-500"><CalendarDays className="size-4" /> Monthly summary</div><span className="rounded-full bg-[#e9f0e4] px-2 py-1 text-[10px] font-bold text-[#31523b]">Live plan</span></div>
          ...
          <div className="mt-4 grid grid-cols-2 gap-3"><div><p className="text-xs text-stone-500">Planned inflow</p><p className="mt-1 font-mono-num text-sm font-semibold text-stone-900 dark:text-white">{formatTZS(latestMonth?.plannedIncome || 0)}</p></div><div><p className="text-xs text-stone-500">Net cushion</p><p className="mt-1 font-mono-num text-sm font-semibold text-emerald-700 dark:text-emerald-400">{formatTZS(masterCalc.monthlyCashFlowAfterDebt)}</p></div></div>
        </section>
      </div>
